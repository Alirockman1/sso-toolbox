...
%		- 'TrueLabelOptions' : 'plot' options which apply to the plot of 'true'
%		label points
%		- 'FalseLabelOptions' : 'plot' options which apply to the plot of 
%		'false' label points
%		- 'SupportVectorOptions' : 'plot' options which apply to the plot of the
%		support vectors
...
%
%   PLOTHANDLE = plot_svm_training_data_2d(...) returns the handles of the
...
%       - PLOTHANDLE : (1,3) Line
%
%   See also plot, ClassificationSVM, legend, plot_svm_training_data_3d.
%
...
%   limitations under the License.
    ...
    plotHandle(2) = plot(svm.X(~label,1), svm.X(~label,2),'LineStyle','none',falseOptions{:});

    % Re-scale the support vectors and plot them
    supportVector = svm.SupportVectors.*svm.Sigma + svm.Mu;
    plotHandle(3) = plot(supportVector(:, 1), supportVector(:, 2),'LineStyle','none',supportVectorOptions{:});
