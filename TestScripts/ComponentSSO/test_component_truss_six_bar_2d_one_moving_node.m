...
% initial truss
plot_results_truss_six_bar_2d_one_moving_node(baseNode,nodeElement,...
...
nodePositionInitial(4,:) = initialDesign;
...
nodePositionOptimized(4,:) = nodePositionOptimal;
...
%% measure comparison
measureComponent = componentSolutionSpace(1).Measure;
measureBox = prod(solutionSpaceBox(2,[1,2])-solutionSpaceBox(1,[1,2]));
...
function figureHandle = plot_results_truss_six_bar_2d_one_moving_node(baseNode,nodeElement,varargin)
    parser = inputParser;
    ...
    if(~isempty(options.NodePositionInitial))
        nodePositionInitial = baseNode;
        nodePositionInitial(4,:) = options.NodePositionInitial;
    ...
    if(~isempty(options.NodePositionOptimal))
        nodePositionOptimized = baseNode;
        nodePositionOptimized(4,:) = options.NodePositionOptimal;
    ...
    if(~isempty(options.BoxSolutionSpace))
        handleToleranceNodeBox = plot_design_box_2d(gcf,options.BoxSolutionSpace(:,[1,2]),boxSolutionOptions{:});
    end

    % component solution space
    handleToleranceNodeComponent = [];
    if(~isempty(options.ComponentSolutionSpace))
        handleToleranceNodeComponent = options.ComponentSolutionSpace(1).plot_candidate_space(gcf,componentSolutionOptions{:});
    end
    ...
    if(~isempty(options.NodePositionRandom))
        ...
        for i=1:nRandomTruss
            ...
            randomNodePosition(4,:) = options.NodePositionRandom(i,[1,2]);
