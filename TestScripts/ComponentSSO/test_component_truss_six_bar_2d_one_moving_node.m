...
designSpaceUpperBoundMass = [2.0  1.5];
Components = {[1,2]'};
%
...
% initial truss + optimized displacment truss
plot_results_truss_six_bar_2d_one_moving_node(baseNode,nodeElement,...
    'NodePositionInitial',initialDesign,...
...
nodePositionOptimized(4,:) = nodePositionOptimalDisplacement;
...
%% measure comparison
measureComponent = componentSolutionSpaceDisplacement(1).Measure;
measureBox = prod(solutionSpaceBoxDisplacement(2,[1,2])-solutionSpaceBoxDisplacement(1,[1,2]));
...
function figureHandle = plot_results_truss_six_bar_2d_one_moving_node(baseNode,nodeElement,varargin)
    ...
    if(~isempty(options.NodePositionOptimalDisplacement))
        nodePositionOptimized = baseNode;
        nodePositionOptimized(4,:) = options.NodePositionOptimalDisplacement;
    ...
    if(~isempty(options.NodePositionOptimalMass))
        nodePositionOptimized = baseNode;
        nodePositionOptimized(4,:) = options.NodePositionOptimalMass;
    ...
    if(~isempty(options.BoxSolutionSpaceDisplacement))
        handleToleranceNodeDisplacementBox = plot_design_box_2d(gcf,options.BoxSolutionSpaceDisplacement(:,[1,2]),boxSolutionDisplacementOptions{:});
    end

    % box-shaped solution space - mass
    handleToleranceNodeMassBox = [];
    if(~isempty(options.BoxSolutionSpaceMass))
        handleToleranceNodeMassBox = plot_design_box_2d(gcf,options.BoxSolutionSpaceMass(:,[1,2]),boxSolutionMassOptions{:});
    end
    ...
    if(~isempty(options.ComponentSolutionSpaceDisplacement))
        handleToleranceNodeDisplacementComponent = options.ComponentSolutionSpaceDisplacement(1).plot_candidate_space(gcf,componentSolutionDisplacementOptions{:});
    end

    % component solution space - mass
    handleToleranceNodeMassComponent = [];
    if(~isempty(options.ComponentSolutionSpaceMass))
        handleToleranceNodeMassComponent = options.ComponentSolutionSpaceMass(1).plot_candidate_space(gcf,componentSolutionMassOptions{:});
    end
    ...
    if(~isempty(options.NodePositionRandomDisplacement))
        ...
        for i=1:nRandomTruss
            ...
            randomNodePosition(4,:) = options.NodePositionRandomDisplacement(i,[1,2]);
    ...
    if(~isempty(options.NodePositionRandomMass))
        ...
        for i=1:nRandomTruss
            randomNodePosition = baseNode;
            randomNodePosition(4,:) = options.NodePositionRandomMass(i,[1,2]);
