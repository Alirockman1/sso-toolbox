function performanceMeasure = truss_six_bar_2d_one_moving_node(designSample,systemParameter)
%TRUSS_SIX_BAR_2D_ONE_MOVING_NODE Bottom-Up Mapping (2D Node Position Problem)
%	TRUSS_SIX_BAR_2D_ONE_MOVING_NODE uses a pre-defined truss and determines the
%	total vertical displacement of the tip, total mass of the structure and 
%	stresses in the bars for different positions of the one free node (top 
%	right):
%	o---0
%	   /|\
%	  /	| o | 
...
%
%	PERFORMANCEMEASURE = TRUSS_SIX_BAR_2D_ONE_MOVING_NODE(DESIGNSAMPLE,
%	SYSTEMPARAMETER) receives the positions of the node in DESIGNSAMPLE and the 
%	cross-section area, Young's modulus and density of the bars in 
...
%			-- (1) : horizontal position of free node (nx)
%			-- (2) : vertical position of free node (ny)
