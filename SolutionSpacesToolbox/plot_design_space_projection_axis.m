function designTypeHandle = plot_design_space_projection_axis(plotData,varargin)
    parser = inputParser;
    parser.addOptional('CurrentAxis',gca,@(x)isgraphics(x));
    ...
    %% Plotting
    activate_graphics_object(parser.Results.CurrentAxis);
    hold on;

    % physically infeasible designs
    if(any(~isPhysicallyFeasible))
        designTypeHandle.PhysicallyInfeasible = plot(...
    ...
    if(any(isGoodPerformance & isPhysicallyFeasible))
        designTypeHandle.GoodPerformance = plot(...
    ...
    for i=1:nRequirement
        ...
        if(~isempty(iViolated))
            designTypeHandle.BadPerformance(i) = plot(...
    ...
    % box contour
    plot_design_box_2d(parser.Results.CurrentAxis,designBox,plotOptionBox{:});
    ...
    if(~isempty(options.AxesLabels))
        xlabel(options.AxesLabels{1});
        ylabel(options.AxesLabels{2});
    end
    grid('off');
    drawnow;
