function designTypeHandle = plot_design_space_projection_axis(dataManager, axesIdx, varargin)
...
%   SPDX-License-Identifier: Apache-2.0
    ...
    %% Extract Data
    plotData = dataManager.PlotData(axesIdx);
   ...
   hold(currentAxis, 'on');
    ...
    for i=1:nRequirement
        ...
        if(~isempty(iViolated))
            designTypeHandle.BadPerformance(i) = plot(currentAxis,...
                ...
                'Color',violateColor,...
                plotOptionsBad{:},...
                'DisplayName', dataManager.QuantatiesOfInterests(i).viol, ...
                'ButtonDownFcn', @(src,event) select_data_from_plot(src,event,options.DataManager), ...
                'PickableParts', 'all', ...
    ...
    if(options.PlotIntervals)
        vLineLeft = plot(currentAxis,...
            [designBox(1,1) designBox(1,1)],...
            [designSpaceLowerBound(2) designSpaceUpperBound(2)],...
            plotOptionsIntervals{:}); % vertical left line

        % Hide from legend
        vLineLeft.Annotation.LegendInformation.IconDisplayStyle = 'off';

        hold(currentAxis, 'on');


        vLineRight = plot(currentAxis,...
            [designBox(2,1) designBox(2,1)],...
            [designSpaceLowerBound(2) designSpaceUpperBound(2)],...
            plotOptionsIntervals{:}); % vertical right line

        vLineRight.Annotation.LegendInformation.IconDisplayStyle = 'off';

        hold(currentAxis, 'on');

        hLineLower = plot(currentAxis,...
    ...
    if(~isempty(options.AxesLabels))
        xlabel(currentAxis, [dataManager.DesignVariables(options.AxesLabels(1)).dispname...
            ' (' dataManager.DesignVariables(options.AxesLabels(1)).unit ')']);
        ylabel(currentAxis, [dataManager.DesignVariables(options.AxesLabels(2)).dispname...
            ' (' dataManager.DesignVariables(options.AxesLabels(2)).unit ')']);
    end
